 ...
 --   hSeek hndl AbsoluteSeek 15
    ...
    if b
        then do
            ...
            prg <- arr `readArray` 4 -- :: Word8
            let prgSize = 16 + (fromIntegral prg)*1024*16
            ...
            chr <- arr `readArray` 5
            let chrSize = prgSize + (fromIntegral chr)*1024*8
...
writeBinToMem' :: [Operand] -> Address -> PPU s ()
writeBinToMem' ops offset = do
    let bs = zip [offset..] ops -- ++ zip   [0x7000..] [0x20, 0x00, 0x70]
    mapM_ (\(a,op) -> writePPUMem a op) bs

writeBinToMemST :: STArray s Address Operand -> [Operand] -> Address -> ST s ()
