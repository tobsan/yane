...
-- | Z,C,N = M * 2
asl :: Address -> CPU s ()
asl addr = alterWithC (alterMem addr) ((`clearBit` 0) . (`shiftL` 1)) >>= setZN

-- | Arithmetic Shift Left
-- | A,Z,C,N = A * 2
asla :: CPU s ()
asla = alterWithC alterA ((`clearBit` 0) . (`shiftL` 1)) >>= setZN
...
-- | A & M, N = M7, V = M6
bit :: Operand -> CPU s ()
bit val = do setFlagV $ testBit val 6
             setFlagN $ testBit val 7
             getA >>= \acc -> setFlagZ $ (acc .&. val) == 0
...
-- | M,C,Z,N = M/2
lsr :: Address -> CPU s ()
lsr addr = alterWithC (alterMem addr) ((`clearBit` 7) . (`shiftR` 1)) >>= setZN

-- | Logical Shift Right
-- | A,C,Z,N = A/2
lsra :: CPU s ()
lsra = alterWithC alterA ((`clearBit` 7) . (`shiftR` 1)) >>= setZN
...
-- | A,Z,N = M(SP)
pla :: CPU s ()
pla = pull >>= \val -> setA val >> setZN val
...
rola = do
    stat <- getFlagC
    op <- getA
    let res = if stat then setBit (shiftL op 1) 0 else shiftL op 1
    setA res >> setZN res
    setFlagC $ op `testBit` 7
...
rol addr =  do
    stat  <- getFlagC
    val <- readMemory addr
    let res = if stat then setBit (shiftL val 1) 0 else shiftL val 1
    writeMemory addr res >> setZN res
    setFlagC $ val `testBit` 7
...
rora =  do
    stat <- getFlagC
    op <- getA
    let res = if stat then setBit (shiftR op 1) 7 else shiftR op 1
    setA res >> setZN res
    setFlagC $ testBit op 0
...
ror addr =  do
    val <- readMemory addr
    stat <- getFlagC
    let res = if stat then setBit (shiftR val 1) 7 else shiftR val 1
    writeMemory addr res >> setZN res
    setFlagC $ val `testBit` 0
...
-- | SP = X
txs :: CPU s ()
txs = getX >>= alterSP . const . fromIntegral >>= setZN . fromIntegral
